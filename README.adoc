...
MicroProfile Telemetry automatically instruments Jakarta RESTful Web Services and MicroProfile REST clients. To trace extra operations, such as internal logic or calls to external systems like a database, you can add manual instrumentation to the source code.
...
The [hotspot=listMethod file=0]`list()`, [hotspot=addMethod file=0]`add()`, and [hotspot=updateMethod file=0]`update()` methods are annotated with the [hotspot=listWithSpan hotspot=addWithSpan hotspot=updateWithSpan file=0]`@WithSpan` annotation. The [hotspot=listWithSpan hotspot=listMethod file=0]`list()` method uses its method name as the default span name, while the [hotspot=addMethod file=0]`add()` and [hotspot=updateMethod file=0]`update()` methods define custom span names, [hotspot=addWithSpan file=0]`Inventory Manager Add` and [hotspot=updateWithSpan file=0]`Inventory Manager Update`, respectively. Each time one of these methods is called, the instrumentation automatically creates a new span.

To enrich spans with more context, you can annotate method parameters with the [hotspot=spanAttribute1 hotspot=spanAttribute2 file=0]`@SpanAttribute` annotation. In this example, the `host` parameter in the [hotspot=addMethod file=0]`add()` and [hotspot=updateMethod file=0]`update()` methods is labeled as [hotspot=spanAttribute1 hotspot=spanAttribute2 file=0]`hostname` in the span.
...
The OpenTelemetry [hotspot=tracer file=0]`Tracer` bean is injected into the [hotspot file=0]`InventoryResource` class to manually instrument your code for trace collection. Before the [hotspot=manager hotspot=getHealth file=0]`InventoryManager` calls the `system` service to check its health status, it creates and starts a span that is named [hotspot=getPropertiesSpan file=0]`GettingProperties`.

Each span must be ended by calling [hotspot=end file=0]`end()`. To ensure that the span always completes, the call is placed inside a [hotspot=finally file=0]`finally` block.

Calling the [hotspot=scope file=0]`makeCurrent()` method sets the span as the current span. This method returns a `Scope` that must be closed to restore the previous context. A [hotspot=try file=0]`try-with-resources` block is used so the scope closes automatically.

Calling the [hotspot=addEvent1 hotspot=addEvent2 file=0]`addEvent()` method adds events to the span. In this example, the code adds one event when the properties are successfully received, and another when the request fails.
...
image::inventory_received_properties_event.png[Inventory Manager Received properties event,align="center",align="center"]
{empty} +
...
The OpenTelemetry [hotspot=meter file=0]`Meter` bean is injected into the [hotspot file=0]`HealthCheckScheduler` class to define and collect application-specific metrics. This interface provides access to the OpenTelemetry metrics API, including instruments such as counters, histograms, and gauges to monitor application behavior and performance.

The [hotspot=counterBuilder file=0]`Meter.counterBuilder()` method defines a counter instrument named [hotspot=schedulerRunCounter file=0]`inventory.health.check.scheduler.runs`. This metric records the total number of health check scheduler executions over time.

The [hotspot=histogramBuilder file=0]`Meter.histogramBuilder()` method defines a histogram instrument named [hotspot=schedulerDuration file=0]`inventory.health.check.scheduler.duration`. This metric measures the time taken for each scheduled health check that is run. The [hotspot=start file=0]`start` time is captured when the [hotspot=performHealthChecks file=0]`performHealthChecks()` method begins, and the [hotspot=duration file=0]`duration` is calculated and recorded when the run completes.
...
The OpenTelemetry [hotspot=meter file=1]`Meter` bean is injected and used to define a gauge instrument by using the [hotspot=gaugeBuilder file=1]`Meter.gaugeBuilder()` method. The gauge is named [hotspot=inventorySize file=1]`inventory.size` and reports the current number of systems in the inventory. The [hotspot=buildWithCallback file=1]`buildWithCallback()` method updates the gauge dynamically so that it always reflects the latest inventory size.
...
Note the difference between the metric name that you define and the name that is displayed in Grafana. For details on how OpenTelemetry metrics are transformed when exported to Prometheus, see the https://opentelemetry.io/docs/specs/otel/compatibility/prometheus_and_openmetrics/#otlp-metric-points-to-prometheus[OTLP Metric points to Prometheus^] section of the OpenTelemetry specification.

Because the health check scheduler runs every [hotspot=scheduleAnnotation file=0]`30` seconds, you can wait a few minutes and refresh the dashboard to watch the metrics change in real time.
...
Because you started Open Liberty in dev mode, run the tests for the `system` and `inventory` services by pressing the `enter/return` key in the command-line sessions where you started the services.
